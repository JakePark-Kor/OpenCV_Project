...
while True:
    ...
    #움직임이 없다하여 mag(magnitude 값이 0이 나오지는 않음! 조명들이 반짝반짝 거리면서 거리를 이동)
    ...
    # 2 픽셀 이상인 것에 대해서만 계산, 잘 못 계산된 노이즈 벡터를 없애기 위한 용도

    mx = cv2.mean(vx, mask=motion_mask)[0] # x 방향성의 평균값
    my = cv2.mean(vy, mask=motion_mask)[0] # y 방향성의 평균값
    m_mag = math.sqrt(mx*mx + my*my)

    if m_mag > 4.0: # 충분한 움직임을 취한 것에 대해서만 계산을 해줌
        m_ang = math.atan2(my, mx) * 180 / math.pi # -180 ~ + 180 
        m_ang += 180 # 0 ~ 360

        pt1 = (100, 100) # 화살표 시작점 좌표

        if m_ang >= 45 and m_ang < 135:
            pt2 = (100, 30) # 화살표 끝 점 좌표
